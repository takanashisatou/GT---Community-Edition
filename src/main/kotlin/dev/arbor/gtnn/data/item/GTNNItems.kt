...
object GTNNItems {
    ...
    fun <T : ComponentItem> registerItemWithTooltip(
        ...
        return GTNNRegistries.REGISTRATE.item(name, factory).onRegister(GTItems.attach<T?>(TooltipBehavior {
            if (num <= 0) return@TooltipBehavior

            val modId = GTNNRegistries.REGISTRATE.modid
            if (num == 1) {
    ...
    fun <T : ComponentItem> registerItemWithTooltip(
        ...
        return GTNNRegistries.REGISTRATE.item(name, factory)
            .onRegister(GTItems.attach<T?>(TooltipBehavior(Consumer { lines ->
                ...
                if (num == 1) {
                    lines!!.add(
                        Component.translatable("item.%s.%s.desc".format(GTNNRegistries.REGISTRATE.modid, name))
                    )
                    ...
                    for (i in 0..<num) {
                        lines!!.add(
                            Component.translatable(
                                "item.%s.%s.desc.%s".format(GTNNRegistries.REGISTRATE.modid, name, i)
                            )
                        )
                    }
                    lines!!.addAll(listOf(*other))
