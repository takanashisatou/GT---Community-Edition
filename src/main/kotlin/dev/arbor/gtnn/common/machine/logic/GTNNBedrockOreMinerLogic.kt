package dev.arbor.gtnn.common.machine.logic
...
class GTNNBedrockOreMinerLogic<T : IRecipeLogicMachine>(machine: T) : RecipeLogic(machine) {
    ...
    override fun findAndHandleRecipe() {
        ...
        if (level is ServerLevel) {
            ...
            if (match != null) {
                ...
                if (RecipeHelper.matchRecipe(this.machine, copied).isSuccess && RecipeHelper.matchTickRecipe(this.machine,  copied).isSuccess) {
                    setupRecipe(match)
                }
            }
        }
    ...
    override fun onRecipeFinish() {
        ...
        if (match != null) {
            val copied = match.copy(ContentModifier(match.duration.toDouble(), 0.0))
            if (RecipeHelper.matchRecipe(this.machine, copied).isSuccess &&
                RecipeHelper.matchTickRecipe(this.machine, copied).isSuccess) {
                setupRecipe(match)
                return
            }
        }
